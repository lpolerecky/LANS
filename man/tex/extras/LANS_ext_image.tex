...
As an example, we will use two input files: \ttt{Ecoli-Azo-Gam42a.im.zip} contains the raw NanoSIMS data (secondary ions of \ttt{12C}, \ttt{13C}, \ttt{19F}, \ttt{12C14N}, \ttt{32S} and \ttt{Esi}, Fig.~\ref{fig:LANS-ext-raw}A), while \ttt{Ecoli-Azo-Gam42a-FISH.tif} contains the FM data (green channel is the fluorescence intensity from a~microbe-specific FISH probe, blue channel is the fluorescence intensity of the DAPI stain attached to DNA of all microbes, Fig.~\ref{fig:LANS-ext-raw}B). These data are available from the same location as the LANS program (folder \ttt{test\_data/NS+FISH}).
...
\sbx{In the main LANS window, process the NanoSIMS dataset \ttt{Ecoli-Azo-Gam42a.im.zip} until you arrive at accumulated and scaled ion count images, as shown in Fig.~\ref{fig:LANS-ext-raw}A (see Section~\ref{sec:level1} for the relevant details).}

\begin{figure}[!ht]
\centering
\begin{tabular}{c@{\hskip0mm}c}
...
\sbx{Export the ion count images or RGB overlays using \lans{Output} $\ra$ \lans{Display masses}.}

\nnb{The choice depends on what you think is the best NanoSIMS image, or an RGB overlay of NanoSIMS images, based on which you will be able to align the external image.}
...
\sbx{Select \lans{External} $\ra$ \lans{Align external and nanosims images}.}
...
\nnb{In this example, select the file \ttt{Ecoli-Azo-Gam42a-FISH.tif}, which looks as shown in Fig.~\ref{fig:LANS-ext-raw}C.}

\s{In the same window, select \lans{File} $\ra$ \lans{Load nanosims image}. Navigate to the file you chose in step~S2.}

\nnb{In this example, select the file \ttt{log(12C14N)-vs-log(32S)-vs-log(19F)-rgb.tif}.}
...
\nnb{Again, use arrow keys to fine-tune the point's position and press \ttt{Enter} to confirm it.}

\sbx{Repeat steps S6 and S7 several times to define additional points in the external image and the corresponding points in the NanoSIMS image, one pair of points at a~time.}
...
\nb{Note that each added point will have a~unique identification number. You will be able to see which points correspond to each other in the external and NanoSIMS images based on these identification numbers.}
...
\nnb{Observe messages in the Matlab console for more details about what exactly you need to do.}
...
\s{After defining a reasonably high number of point pairs (typically 5--10 pairs should be sufficient), select \lans{File} $\ra$ \lans{Save point list} to save the points' coordinates in a~matlab file. }
 
\nnb{You can load these coordinates later via \lans{File} $\ra$ \lans{Load point list}. Subsequently, you can add more points, modify points, or remove points, as described in steps S6--S10.}

\s{If the selected NanoSIMS image does not contain all features based on which you can align the external image, you can load a~\bb{different} NanoSIMS image at any time (see step~S5). When doing so, you may need to select \lans{Action} $\ra$ \lans{Update points} to redraw the currently defined points in the image.}
...
\s{If you are satisfied with the result, select \lans{Action} $\ra$ \lans{Export aligned external image} to \bb{store the aligned external image} for later use. If not, repeat steps~S6--S13.}

\nb{When storing the aligned external image, specify an appropriate name (e.g., \ttt{ext\_aligned}) and location of the file. You can store it in a~Matlab (\ttt{*.mat}) or TIF (\ttt{*.tif}) format. It is a~good idea to store them in both formats and in the folder with all the other processed data.}
...
\nnb{This will reassure you that the external image is really usable as part of the NanoSIMS dataset.}

\s{To use the external image for defining ROIs, type \ttt{ext} in the \lanstf{ROI definition template} and proceed as described in Section~\ref{sec:defineROIs} to define ROIs.}

\nnb{This is an example where \lans{Interactive thresholding} (Section~\ref{sec:defineROIsInteractive}) will really speed up the  ROI definition.}
...
\sbx{Classify the ROIs using steps described in Section~\ref{sec:classifyROIs}.}

\nnb{You can base this classification on the aligned external image, where you clearly see (in most cases) the differences between cells only stained with DAPI (blue) and cells stained with both DAPI and the FISH probe (cyan). You can use, for instance, \ttt{b} and \ttt{c} as the class names for the respective cells.}

\sbx{Type \ttt{ext/pixel} into one of the ratio \lanstf{expression} text fields and specify the \lanstf{scale}.}

\nnb{This is necessary if you want to properly normalize the ROI-specific values for the external image data.}

\sbx{Create overlays between the external image and one of the NanoSIMS images, or scatter plots between the ROI-specific values in the external and NanoSIMS images, using steps described in Section~\ref{sec:level1}.}

\nnb{Examples of the results may look as shown in Fig.~\ref{fig:LANS-ext-overlays}.}
...
\textsl{\small Spataro et al.~(2023). Stable isotope labeling and ultra-high-resolution NanoSIMS imaging reveal alpha-synuclein-induced changes in neuronal metabolism in vivo. \emph{Acta Neuro\-pathol Commun}~\bb{11}:157. \url{https://doi.org/10.1186/s40478-023-01608-8}}
\end{minipage}
\end{center}
\tcbe

\noindent 
You are encouraged to read the supplement of the publication to learn in detail how to resample nanoSIMS images in LANS and then use a~high-resolution external image as a~template for ROI definition for such a~resampled nanoSIMS dataset. A~copy of the supplement, as well as the related example data, can be downloaded from the same location as the LANS program (folder \ttt{test\_data/NS+TEM}). The datasets include the raw NanoSIMS data (\ttt{Spataro-3.im.zip}) and the corresponding TEM image (\ttt{Spataro-3\_TEM.tif}). 
