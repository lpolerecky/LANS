function p=read_cameca_image(fname, ask_for_planes, load_accumulated)
% Read data from the Cameca Image file FNAME. The IM file can be zipped. 
% USAGE:
%    im=read_cameca_image(fname,ask_for_planes)
% Required input: 
%   - FNAME: full filename 
...
if exist(fname)==2           
    [im, planes, p]=read_im_file(fname,ask_for_planes,load_accumulated);
    log_user_info(fname);        
    p.im = im;
    p.planes = planes; 
end;
