...
opt1=[my_get(handles.checkbox12,'value'),... % 1 include cell outline in final
    ...
    my_get(handles.checkbox63,'value')];     % 16 Modulate hue by intensity
...
if(flag==1)
    ...
    if length(handles.p.mass)>length(opt4)
        % this happens when ext image was loaded
        opt4 = [opt4 ones(1,length(handles.p.mass)-length(opt4))];
    end
    ...
    if length(opt4)>length(opt3)
        opt3 = [opt3 opt3(8)*ones(1,length(opt4)-length(opt3))];
    end
