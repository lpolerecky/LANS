function test_anova(x,g,r,cls,tt,metafile,ax,compare,handles,ef)
% use ANOVA to test for differences in means (note that if
% the Levene's test fails to show that variances are
% homogeneous, these results should not be used for interpretation)
...
if length(g)==length(unique(g))
    ...
    fprintf(1,'%s\t%.3e\t%.3e\t%.3e\t%d\n','all',mean(x),std(x)/sqrt(length(x)-1),std(x),length(x));
    
    % display ANOVA results
    fprintf(1,'ANOVA TABLE:\n');
    fprintf(1,'source of variability\tSS      \tdf\tMSS=SS/df  \tsqrt(MSS)\tSSi/SStot\n','%')
    it=2;
    fprintf(1,'%s\t%.3e\t%d\t%.3e\t%.3e\t%.2f\n','Between groups (SST)', table{it,2},table{it,3},...
        table{it,2}/table{it,3},sqrt(table{it,2}/table{it,3}),table{it,2}/table{4,2});
    it=3;
    fprintf(1,'%s\t%.3e\t%d\t%.3e\t%.3e\t%.2f\n','Within groups (SSE)', table{it,2},table{it,3},...
        table{it,2}/table{it,3},sqrt(table{it,2}/table{it,3}),table{it,2}/table{4,2});
    it=4;
    fprintf(1,'%s\t%.3e\t%d\t%.3e\t%.3e\t%.2f\n','Total = all (SStot)', table{it,2},table{it,3},...
        table{it,2}/table{it,3},sqrt(table{it,2}/table{it,3}),table{it,2}/table{4,2});
    fprintf(1,'F(%d,%d):\t%.3e\n',table{2,3},table{3,3},table{2,5});
    fprintf(1,'Prob>F:\t%.2e\n',table{2,6});
    ...
    if ~isnan(p) % p is NaN when there is only one datapoint
        ...
        if p>0.05
            title(sprintf('Means do not significantly differ (ANOVA, p=%.2e)',p),...
				'Fontsize',defFontSize,'fontweight','normal');
        else
            title(sprintf('Means significantly differ (ANOVA, p=%.2e)',p),...
				'Fontsize',defFontSize,'fontweight','normal');
        end;
    ...
    if 1 & ef
        ...
        a = construct_output_fname(metafile,r,compare,'tests','anova','.txt');
        ...
        if 0 & isunix
            ...
            fprintf(1,'Warning: this is where Matlab can crash. test_anova.m: 74\n');
        ...
        if length(PathName)>0 % if cancel was not pressed
            ...
            if ~isnan(p) % p is NaN when there is only one datapoint
                ...
                if p>0.05
                    fprintf(fid,'Means DO NOT significantly differ amongst %s (ANOVA, p=%.2e).\n',compares,p);            
                else
                    fprintf(fid,'Means significantly differ amongst %s (ANOVA, p=%.2e).\n',compares,p);
                end;

                % print ranking results
                fprintf(fid,'\nRanking based of multiple comparison of the means:\n');
                fprintf(fid,'------------------------------------------------------------------------\n');
                ...
                ranking_table_explanation(fid,'MEAN',compare,[1 1]);
            end;
