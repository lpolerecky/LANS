function varargout = depth_profiles_ratios_gui(varargin)
% DEPTH_PROFILES_RATIOS_GUI MATLAB code for depth_profiles_ratios_gui.fig
%      DEPTH_PROFILES_RATIOS_GUI, by itself, creates a new DEPTH_PROFILES_RATIOS_GUI or raises the existing
%      singleton*.
%
%      H = DEPTH_PROFILES_RATIOS_GUI returns the handle to a new DEPTH_PROFILES_RATIOS_GUI or the handle to
%      the existing singleton*.
%
%      DEPTH_PROFILES_RATIOS_GUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DEPTH_PROFILES_RATIOS_GUI.M with the given input arguments.
%
%      DEPTH_PROFILES_RATIOS_GUI('Property','Value',...) creates a new DEPTH_PROFILES_RATIOS_GUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before depth_profiles_ratios_gui_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to depth_profiles_ratios_gui_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help depth_profiles_ratios_gui

% Last Modified by GUIDE v2.5 03-Jul-2011 12:57:54

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @depth_profiles_ratios_gui_OpeningFcn, ...
                   'gui_OutputFcn',  @depth_profiles_ratios_gui_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before depth_profiles_ratios_gui is made visible.
function depth_profiles_ratios_gui_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to depth_profiles_ratios_gui (see VARARGIN)
...
handles = display_selected_ratio(hObject,handles,1);
...
% UIWAIT makes depth_profiles_ratios_gui wait for user response (see UIRESUME)
...
function handles = display_selected_ratio(hObject, handles,ri)
...
if ~isempty(handles.data)
    ...
    if get(handles.checkbox102,'value')
        ylabel(sprintf('%s: mean +/- Poisson error',data.ratio{ri}),'FontSize',defFontSize);
    else
        ylabel(sprintf('%s: mean',data.ratio{ri}),'FontSize',defFontSize);
    end;
...
function varargout = depth_profiles_ratios_gui_OutputFcn(hObject, eventdata, handles) 
...
function toggle_profile(handles)
global additional_settings;
sel=get(handles.listbox1,'value');
...
% visible again
make_handles_visible(handles,'off');
ri = get(handles.popupmenu1,'value');
...
make_handles_visible(handles,'on');

function make_handles_visible(handles,vis)
