...
\begin{figure}[!ht]
\centering
\includegraphics[scale=0.45]{figs1/LANS-dt-qsa-settings}
...
\s{Select \lans{Input} $\ra$ \lans{Display plane images for all masses} to view secondary ion counts detected in individual planes for \bb{all} masses.}

\purplebox{}
This view shows the actual data in its \bb{rawest} form, i.e., \bb{ion counts per pixel per plane.}
\tcbe
...
\sbx{In the new window that opens (Fig.~\ref{fig:displayplanes}), use arrows to browse through the planes forward and backward.}
...
\sbx{Close the window when you have defined the area.}
...
\sbx{Select \lans{Input} $\ra$ \lans{Accumulate plane images} to start the accumulation of drift-corrected images. }
...
\nnb{If you \lans{reject} it, you can reiterate the previous steps (S1--S4) until you arrive at a dataset with drift-corrected and accumulated planes.}
...
\nnb{If you are unable to align the planes in a~satisfying manner, the best you can do is to accumulate them without the drift-correction. This is done by unchecking \lanscb{Align images when accumulating} before plane accumulation.}
...
\purplebox{Remember}
When displaying the images, ensure that \lanscb{Export PDF graphics} in the \ttt{Output options} box of the main LANS window is checked. This will allow you to \bb{export} the images in a~PDF format (they will be stored in the \ttt{pdf} sub-folder of the current data folder). If this checkbox is not checked, no output will be generated, and you may be left wondering: why doesn't it work?!
\tcbe
...
\sbx{Explore the \lans{Action} menu to test various ways to \lans{define}, \lans{split}, \lans{merge}, or \lans{remove} ROIs.}

\sbx{Use the \lans{Zoom} menu to zoom in the ROI definition image or zoom out.}
...
\sbx{Select \lans{Save ROIs} from the menu, or press \ttt{Ctrl+s}, to save the currently defined ROIs. }
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.98\textwidth]{figs3/LANS-scatter-plots}
...
\s{Select \lans{Preferences} $\ra$ \lans{Store preferences} to \textcolor{red}{save the settings} of the current data processing session in a file (it is recommended to use the file name \ttt{prefs.mat}). }

\nb{This is useful when you want to return to the analysis of the same file in the future, and essential if you want to later combine results of your analyses, or perform automatic analyses of multiple datasets via ``metafile processing'', as described in Section~\ref{sec:level2}. Therefore, it is highly recommended that you \textcolor{red}{do this} for every processed dataset.}
...
\sbx{Select \lans{Preferences} $\ra$ \lans{Backup (full) folder with processed data}.}
...
\sbx{Alternatively, select \lans{Preferences} $\ra$ \lans{Backup (minimal) folder with processed data}.}
