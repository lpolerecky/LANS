...
if(~isempty(q))
    ...
    if(~isempty(fn) & exist(fn)==2)
        [meanval,r]=readdacfile(fn,1,verbose);
        ...
        if (exist(fncells)==2)
            ...
            for ii=1:length(ct)
                ...
                if(~isempty(ind) & size(r,1)==length(cnum))
                   ...
                   l2w{ii}=[r(cnum(ind),9) zeros(length(ind),1)];
            ...
            disp(['** File ',fncells,' not found. All cells treated as equal (class "a").']);
            ii=1; d=[];
            d{ii} = r(:,4:5);
            cellid{ii}=r(:,1);
